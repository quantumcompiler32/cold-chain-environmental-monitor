...
DEFAULT_MANIFEST = PROJECT_ROOT / "tests" / "e2e_scenarios.json"
...
def public_environment() -> dict[str, str]:
    """Report non-secret connection settings used by local services."""
    return {
...
def generator_supports_run_id(python: str, env: dict[str, str]) -> bool:
    ...
    result = subprocess.run(
        [python, "-m", "services.temperature_event_generator", "--help"],
        cwd=PROJECT_ROOT,
        env=env,
        text=True,
        capture_output=True,
...
def main() -> int:
    ...
    try:
        ...
        listener = start_process([python, "-m", "services.temperature_subscriber", "--write-db", "--output-mode", "none"], env=env)
        time.sleep(args.startup_delay)
        if listener.poll() is not None:
            raise RuntimeError(f"temperature subscriber exited with code {listener.returncode}")

        bridge = start_process([python, "-m", "services.dashboard_bridge", "--host", "127.0.0.1", "--port", str(bridge_port)], env=env)
        ...
        for case in manifest["cases"]:
            ...
            command = [python, "-m", "services.temperature_event_generator", "--sensor", "Pod1", "--vaccine", "pfizer_ultralow", "--scenario", case["scenario"], "--count", str(case["count"]), "--interval-ms", "100", "--seed", str(case["seed"]), "--batch-id", batch_id]
