...
| Generator publishes but no rows appear | Listener or database | Confirm the listener includes `--write-db`; inspect its `event_rejected` or `database_write_failed` log; query PostgreSQL directly. |
