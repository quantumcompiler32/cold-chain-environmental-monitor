...
5. Run `python3 db/scripts/verify_persistence.py` and point out current event_time values and ingestion latency.
...
```bash
APP_ENV=demo python3 db/scripts/reset_demo.py --confirm-reset
python3 -m services.temperature_event_generator --scenario normal --count 30 --seed 42 --output-mode summary
python3 db/scripts/verify_persistence.py
